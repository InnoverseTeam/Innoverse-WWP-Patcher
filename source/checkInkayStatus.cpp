...
bool skipPatches() {
	std::string inkayConfigPath = "/plugins/config/inkay-innoverse.json";
	...
	log("Environment path is: %s, WWP-Innoverse config path is %s", environmentPathBuffer, (environmentPathBuffer + inkayConfigPath).c_str());

	FILE* configFile = fopen((environmentPathBuffer + inkayConfigPath).c_str(), "r");
	if (configFile == NULL) {
		log("WWP-Innoverse config file does not exist");
		return true;
	}
	...
	if (size > MAX_BUFFER_SIZE) {
		log("WWP-Innoverse config file is too large");
		return true;
	}
	...
	if (bytesRead == 0) {
		log("WWP-Innoverse config file is empty");
		return true;
	}
	...
	if (result == -1) {
		log("WWP-Innoverse config file is invalid");
		return true;
	}
